...
public class playerMoveCaja : MonoBehaviour
{
    ...
    void Update ()
    {
        if (player.input.RightTrigger.WasPressed)
        {
        ...
        if (controller.box)
        {
            ...
            if (player.input.RightTrigger.WasReleased || Mathf.Abs(Vector3.Distance(controller.box.transform.position, _pos)) > _dis)
            {
